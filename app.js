...
function startFirebaseSync() {
  ...
  db.ref(FB_PATH).on('value', snap => {
    ...
    if (val && typeof val === 'object') {
      ...
      // Sync PIN to localStorage so login works offline too
      if (val.pin) localStorage.setItem('biz_pin', val.pin);
      renderAll();
    } else {
      // Firebase is empty - use local or default
      const local = getData();
...
function updateDue() {
  ...
  if (action !== 'zero' && !amt) { alert('Amount দিন!'); return; }
  const d = getData();
...
function renderSupplier() {
  ...
  if (cards) {
    cards.innerHTML = `
      <div class="due-card">
        ...
        <div style="font-size:11px;color:var(--muted);margin-top:4px;">আমার দেওয়া বাকি</div>
      </div>
      <div class="due-card">
  ...
  tb.innerHTML = log.map(l => {
    ...
    const aLabel = l.action==='add' ? '+ মাল কিনলাম' : l.action==='paid' ? '- Payment দিলাম' : '✓ সব পরিশোধ';
    return `<tr>
      <td style="font-size:12px;">${fmtDate(l.date)}</td>
...
function saveSupplierNames() {
  ...
  if (!n1 || !n2) { if(msg){msg.style.color='var(--accent3)';msg.textContent='দুটো নামই দিন!';} return; }
  const d = getData();
...
function renderBankTransfer() {
  ...
  if (sc) sc.innerHTML = `
    <div class="sum-card"><div class="sum-label">আজকের Transfer</div><div class="sum-val amber">${fmt(todayAmt)}</div></div>
    <div class="sum-card"><div class="sum-label">এই মাসের Transfer</div><div class="sum-val amber">${fmt(monthAmt)}</div></div>
    <div class="sum-card"><div class="sum-label">মোট Transfer</div><div class="sum-val" style="color:var(--accent);">${fmt(totalAmt)}</div></div>
  `;
  renderBankTransferList();
}
...
function restoreBackup(input) {
  ...
  reader.onload = function(e) {
    try {
      ...
      if (!confirm('এই backup restore করলে current data replace হবে। Continue?')) return;
      const d = backup.data;
      setData(d);
      const msg = document.getElementById('backupMsg');
...
function clearAllData() {
  ...
  if (!entered) {
    if (msgEl) { msgEl.style.color='var(--accent3)'; msgEl.textContent='PIN দিন!'; }
    return;
  }
  if (entered !== getPin()) {
    if (msgEl) { msgEl.style.color='var(--accent3)'; msgEl.textContent='Wrong PIN! Data delete হয়নি।'; }
    if (pinEl) pinEl.value = '';
    return;
  }
  if (!confirm('সত্যিই সব data মুছে দেবো? এটা undo করা যাবে না!')) return;
  if (!confirm('শেষবার নিশ্চিত — সব data চিরতরে মুছে যাবে!')) return;
