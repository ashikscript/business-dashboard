...
function setData(d) {
  ...
  db.ref(FB_PATH).set(d).catch(console.error);
}
...
function checkMidnightReset() {
  ...
  if (el) el.textContent = getData().lastReset || '-';
}
function cleanTrash() {
  const d = getData(); const now = Date.now();
...
function renderAll() {
  renderSummary(); renderTodaySales(); renderDue(); renderStock();
  renderTrash(); renderExpenseSummary(); renderExpenseList();
...
function renderSummary() {
  const d = getData(); const today = todayStr(); const month = monthStr();
  ...
  const monthSales = d.sales.filter(s=>s.date&&s.date.startsWith(month));
  ...
  el.innerHTML = `
    ...
    <div class="sum-card"><div class="sum-label">Stock Value</div><div class="sum-val amber">${fmt(d.stock)}</div></div>
...
function openEditSale(id) {
  const d = getData(); const s = d.sales.find(x=>x.id===id);
...
function saveEdit() {
  const d = getData(); const s = d.sales.find(x=>x.id===editingId);
  if (!s) return;
  ...
  setData(d); closeModal('editModal'); renderAll();
}
function deleteSale(id) {
  if (!confirm('Move to trash?')) return;
  const d = getData(); const idx = d.sales.findIndex(s=>s.id===id);
  if (idx<0) return;
  const item = d.sales.splice(idx,1)[0];
  item.deletedAt = Date.now(); item.trashType = 'sale';
  d.trash = d.trash||[]; d.trash.push(item);
  setData(d); renderAll();
}
...
function renderHistory() {
  ...
  if (hs) hs.innerHTML = `<div style="display:flex;gap:10px;flex-wrap:wrap;">
...
function renderDue() {
  ...
  tbody.innerHTML = log.map((l,i) => `<tr>
...
function openEditDue(id) {
  const d = getData(); const l = (d.dueLog||[]).find(x=>x.id===id);
  if (!l) return; editingDueIdx = id;
...
function openEditStock(id) {
  const d = getData(); const l = (d.stockLog||[]).find(x=>x.id===id);
  if (!l) return; editingStockIdx = id;
...
function saveStockEdit() {
  const d = getData(); const l = (d.stockLog||[]).find(x=>x.id===editingStockIdx);
  if (!l) return;
  ...
  setData(d); closeModal('editStockModal'); renderAll();
}
...
function deleteExpense(id) {
  ...
  item.deletedAt=Date.now(); item.trashType='expense';
  d.trash=d.trash||[]; d.trash.push(item);
  setData(d); renderAll();
}
function openEditExp(id) {
  const d = getData(); const e = (d.expenses||[]).find(x=>x.id===id);
  if (!e) return; editingExpId = id;
...
function saveExpEdit() {
  const d = getData(); const e = (d.expenses||[]).find(x=>x.id===editingExpId);
  if (!e) return;
  ...
  setData(d); closeModal('editExpModal'); renderAll();
}
...
function renderReports() {
  const d = getData(); const today = new Date(); let content = '';
  if (reportView==='weekly') {
    ...
    content = `<div class="card" style="margin-bottom:1rem;"><div class="sec-title">Weekly Profit</div><div style="margin-top:12px;">
      ${rows.map(r=>`<div class="bar-row"><div class="bar-label">${r.day.slice(5)}</div>
        ...
        <div class="bar-val">${fmt(r.profit)}</div></div>`).join('')}
    </div></div>
    ...
    content = `<div class="card" style="margin-bottom:1rem;"><div class="sec-title">Monthly Profit</div><div style="margin-top:12px;">
      ${keys.map(k=>`<div class="bar-row"><div class="bar-label">${k}</div>
        ...
        <div class="bar-val">${fmt(months[k].profit)}</div></div>`).join('')}
    </div></div>
    <div class="card"><div class="table-wrap"><table>
...
function restoreTrash(id) {
  const d = getData(); const idx = (d.trash||[]).findIndex(t=>t.id===id);
  if (idx<0) return;
  const item = d.trash.splice(idx,1)[0];
  const type = item.trashType||'sale';
  delete item.deletedAt; delete item.trashType;
  if (type==='expense') { d.expenses=d.expenses||[]; d.expenses.push(item); }
  else { d.sales.push(item); }
  setData(d); renderAll();
}
