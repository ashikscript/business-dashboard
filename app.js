...
// ===== TAB SWITCH =====
const TAB_ORDER = ['today','history','due','stock','expense','reports','trash','settings'];
...
function switchTab(name, btn) {
  ...
  const oldIdx = TAB_ORDER.indexOf(current);
  const newIdx = TAB_ORDER.indexOf(name);
  const goRight = newIdx > oldIdx;

  // Animate out old tab
  if (oldTab) {
    oldTab.style.transition = 'opacity 0.18s ease, transform 0.18s ease';
    oldTab.style.opacity = '0';
    oldTab.style.transform = goRight ? 'translateX(-30px)' : 'translateX(30px)';
    setTimeout(() => {
  ...
  newTab.style.transform = goRight ? 'translateX(30px)' : 'translateX(-30px)';
  newTab.style.transition = 'none';
  newTab.classList.add('active');

  // Render content before animating in
  ...
  requestAnimationFrame(() => {
    requestAnimationFrame(() => {
      newTab.style.transition = 'opacity 0.18s ease, transform 0.18s ease';
